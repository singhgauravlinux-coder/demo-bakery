...
// --- Email and SMS fan-out (mock) ---
app.post('/notify/email', (req, res) => {
  const { to, subject } = req.body || {};
  if (!to) return res.status(400).json({ error: 'recipient (to) is required' });
  req.log.info({ event: 'email_sent', to, subject: subject || '(no subject)' }, 'email dispatched');
  res.status(202).json({ status: 'queued', channel: 'email', to });
});

app.post('/notify/sms', (req, res) => {
  const { to } = req.body || {};
  if (!to) return res.status(400).json({ error: 'recipient (to) is required' });
  req.log.info({ event: 'sms_sent', to }, 'sms dispatched');
  res.status(202).json({ status: 'queued', channel: 'sms', to });
});
