...
// --- Product imagery metadata ---
const media = {
  'p-1': { alt: 'Levain country loaf with a deep scored ear', palette: ['#3b2a1d', '#c89b6a'] },
  'p-3': { alt: 'Cross-section of a butter croissant showing open honeycomb', palette: ['#e8b04b', '#7a4a1f'] },
  'p-8': { alt: 'Rustic sour cherry galette on parchment', palette: ['#7c2230', '#d9b98a'] }
};

app.get('/media/:productId', (req, res) => {
  const item = media[req.params.productId];
  if (!item) return res.status(404).json({ error: 'No media for that product' });
